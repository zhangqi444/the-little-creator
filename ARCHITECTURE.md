...
**Concurrency seam (added 2026-05-26):** the "direct to main" model assumes the agent has exclusive access to `.git/index`, which isn't true when interactive cowork sessions are running against the same checkout. Git serializes write operations through `.git/index.lock`; if an agent's `git commit` collides with a concurrent interactive `git commit`, the agent fails with `fatal: Unable to create '.git/index.lock'` and (without protection) its work gets orphaned in the working tree. The discovery agent's SKILL prompt wraps its commit + push step in a **3-attempt retry with 30s / 60s / 120s backoff**, and on retry-exhaustion it leaves the edits in place + appends an `ORPHAN:` entry to `progress/heartbeat-log.md` so the next maintainer interactive session can pick up the work explicitly. A more thorough fix (agents operate in a `git worktree`, eliminating the lock contention) is a future improvement; the retry+heartbeat pattern covers the common case at ~5 lines of prompt change.
